...
| **E-2** | **FI-005** — Atomicidad de `cuentas_smtp.enviados_hoy` (recuento real unificado) | ✅ **RESUELTO** (2026-08-27, local): nuevos helpers `enviadosHoyDeCuenta()`/`elegirCuentaSmtpDisponible()`/`sincronizarEnviadosHoyCuenta()`; cron (motor + flujo), `enviar_lote.php`, `get_cola.php`, `api/smtp.php` y `enviarRespuestaSmtpLead()` (Bandeja, corregido el bug de columnas `$cuenta['smtp']`/`user`/`pass`) usan recuento real de `comunicaciones_log`. **Extra**: el envío a medida (modal Atender) ya no choca con la idempotencia del envío base (`esMedida` → fila nueva en `envios`) | Evita sobre-envíos por cuenta |
