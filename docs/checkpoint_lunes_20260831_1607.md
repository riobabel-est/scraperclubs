...
> Fecha preparación: 2026-08-31 16:07 · BD: stats.db.remoto_20260831_160714
...
## ✅ ACTUALIZACIÓN 2026-08-31 — TEST DE 10 EJECUTADO (producción)

### Operaciones realizadas
1. **Corrección contador SMTP**: `enviados_hoy` sincronizado al recuento real de hoy (0/0) en la BD subida a producción. Verificado.
2. **Batch en producción**: `2026-08-30-A` (AUTORIZADO, tamaño 200) creado en la BD desplegada. Verificado.
3. **Kit de disparo desplegado** (producción):
   - `outbound/cli/enviar_lote_batch.php` — runner del lote (rotación SMTP por carga mínima; el batch NO se marca ENVIADO en disparos parciales).
   - `public_html/scripts/runner_envio_lead.php` — wrapper CLI (rutas adaptadas a producción).
4. **Bloqueo WAF confirmado**: POST directo a `api/enviar_lote.php` → HTTP 403 (WAF SiteGround, protección legítima). El disparo se ejecuta DENTRO del servidor (cron/runner interno), nunca por HTTP externo.

### Resultado del TEST de 10 (disparado desde el servidor)
```
 LOTE 2026-08-30-A · OK=10 · ERROR=0 · duración=28.6s · batch=AUTORIZADO
```
- 10/10 ACCEPTED (estado `enviado`; lead 21 ya `abierto` en <30s → tracking OK).
- Variantes deterministas correctas: B,B,C,C,A,C,A,C,A,B.
- SMTP rotativo: 1 envío por cuenta (rodrigo→sergio); `enviados_hoy` = 1/1/.../1.
- `es_test=0` (100% REAL), 0 duplicados, tracking_ids únicos (10/10).
- Batch vinculado: `campaign_batch_id='2026-08-30-A'` en los 10 envíos (UPDATE en producción).
- Integridad BD: OK.

### Estado tras el test (producción, verificado)
- Envíos comerciales hoy: **10 / 150** (quedan 140 hoy).
- Batch: AUTORIZADO (200) · pendientes de campaña 2 sin envío: 1425.
- Próximos candidatos para el lanzamiento del usuario: **lead 22 en adelante** (los leads 12-21 ya tienen envío y la idempotencia los excluye).

### Siguiente paso
- El operador lanza sus **150** desde la lanzadera del panel (campaña 2 → cola → iniciar → pausar al llegar a **150**). Los 10 del test ya cuentan dentro del límite diario.
- Tras el lanzamiento: verificar ACCEPTED/ERROR, vincular `campaign_batch_id`, contadores SMTP, motor pausado y generar el informe final (sección 18).
