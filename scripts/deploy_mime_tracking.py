...
# -*- coding: utf-8 -*-
"""
Deploy MIME + TRACKING (FIX DEFINITIVO) — FASE 2/3/4
Sube SOLO:
  - inc/mime.php
  - api/enviar_lote.php

1. Backup remoto de ambos archivos en /getfutprotec.com/backups_deploy/
2. Subida local -> remoto
3. Verificacion MD5 local vs remoto (debe ser MATCH)

NO toca stats.db, campañas, leads, plantillas, SMTP, app.js, lanzadera, abc, eligibilidad, track, cron.
"""
import ftplib
import os
import sys
...
def main():
    ...
    remote_bk = f"{REMOTE_BACKUP_BASE}/mime_tracking_pre_deploy_{ts}"
    print(f"\n=== FASE 2: BACKUP REMOTO en {remote_bk} ===")
    ...
    # ── FASE 4: VERIFICACION MD5 ──
    print("\n=== FASE 4: VERIFICACION MD5 LOCAL vs REMOTO ===")
    all_ok = True
    for rel in DEPLOY_FILES:
        local_path = os.path.join(LOCAL_BASE, rel)
        if not os.path.exists(local_path):
    ...
    with open(os.path.join("backups_deploy", "deploy_mime_tracking_manifest.txt"), "w") as f:
