<?php
/**
 * Atribución retroactiva de respuestas IMAP sin lead_id/envio_id.
 *
 ...
 *
 * USO:
 *   php scripts/atribuir_respuestas_retroactivo.php            # dry-run (solo muestra)
 *   php scripts/atribuir_respuestas_retroactivo.php --apply     # aplica cambios
 *
...
// Ruta de la BD (relativa al script)
$dbPath = __DIR__ . '/../public_html/outbound/data/stats.db';
if (!file_exists($dbPath)) {
    fwrite(STDERR, "ERROR: No se encuentra la BD en {$dbPath}\n");
    exit(1);
}
...
$db->close();
echo "\n" . ($APPLY ? "Cambios APLICADOS." : "DRY-RUN completado. Usa --apply para aplicar.") . "\n";
