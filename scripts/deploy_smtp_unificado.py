...
# -*- coding: utf-8 -*-
"""
Deploy FTP - SMTP UNIFICADO (transporte centralizado)
Sube SOLO los archivos del refactor SMTP unificado a SiteGround.
1. Crea backup remoto de los archivos que se van a sobrescribir (en /getfutprotec.com/backups_deploy/).
2. Sube los archivos del SMTP unificado.
...
# Archivos del refactor SMTP unificado
DEPLOY_FILES = [
...
def main():
    ...
    remote_bk = f"{REMOTE_BACKUP_BASE}/outbound_smtp_unificado_{ts}"
    ...
    for rel in DEPLOY_FILES:
        ...
        try:
            ...
            print(f"  [SKIP] No existe en remoto (nuevo): {rel}")
            continue
    ...
    # ── 2. SUBIR LOS ARCHIVOS DEL SMTP UNIFICADO ──
    print("\n=== 2. SUBIDA SMTP UNIFICADO ===")
    results = []
    for rel in DEPLOY_FILES:
    ...
    with open(os.path.join("backups_deploy", "smtp_unificado_deploy_manifest.txt"), "w") as f:
    ...
    print("Deploy SMTP unificado completado.")
