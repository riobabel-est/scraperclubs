...
<h5 class="text-sm font-bold text-slate-200" x-text="aqTitulo(aqTab)"></h5>
<span x-show="aqData" class="text-xs text-slate-500 ml-2" x-text="aqData.total + ' registros'"></span>
<span x-show="aqData && aqData.hoy !== undefined" class="text-xs text-blue-400 ml-2" x-text="'(hoy: '+aqData.hoy+')'"></span>
<span x-show="aqLoading" class="text-xs text-slate-500 ml-2">Cargando...</span>
</div>
<div class="flex items-center gap-1">
<button @click="abrirAnalytics('envios')" class="px-2.5 py-1 rounded text-xs font-semibold transition" :class="aqTab==='envios'?'bg-blue-500/20 text-blue-400':'text-slate-500 hover:text-slate-300'">Envíos</button>
...
<div class="p-5">
<div x-show="aqLoading" class="flex items-center justify-center py-16"><span class="w-6 h-6 border-2 border-amber-400 border-t-transparent rounded-full animate-spin"></span></div>
<div x-show="!aqLoading && aqData && aqData.ultimos">
...
<th class="px-2 py-1.5 text-left font-semibold" x-show="aqTab!=='bajas'">Fecha</th>
<th class="px-2 py-1.5 text-left font-semibold">Club / Email</th>
<th class="px-2 py-1.5 text-left font-semibold hidden sm:table-cell" x-show="aqTab==='envios'">Asunto</th>
<th class="px-2 py-1.5 text-left font-semibold hidden sm:table-cell" x-show="aqTab==='rebotes'">Motivo</th>
<th class="px-2 py-1.5 text-left font-semibold" x-show="aqTab==='bajas'">Estado</th>
</tr>
</thead>
<tbody>
...
<td class="px-2 py-1">
<span class="text-slate-300" x-text="row.club||row.nombre_club"></span>
<div class="text-slate-500" x-text="row.email"></div>
</td>
<td class="px-2 py-1 text-slate-400 hidden sm:table-cell truncate max-w-[200px]" x-show="aqTab==='envios'" x-text="row.asunto?.substring(0,50)"></td>
<td class="px-2 py-1 text-slate-400 hidden sm:table-cell" x-show="aqTab==='rebotes'" x-text="row.motivo"></td>
<td class="px-2 py-1" x-show="aqTab==='bajas'">
