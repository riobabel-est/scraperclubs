...
<div class="text-center"><span class="text-slate-500 text-xs block">Envios</span><span class="text-sm font-bold text-blue-400" x-text="ld.total_envios||0">0</span></div>
<div class="text-slate-600">|</div>
<div class="text-center"><span class="text-slate-500 text-xs block">Aperturas</span><span class="text-sm font-bold text-cyan-400" x-text="ld.total_aperturas||0">0</span></div>
</div>
</div>
</div>
...
<a :href="waLink" x-show="waLink" target="_blank" class="px-3 py-1.5 rounded-lg text-sm font-semibold transition flex items-center gap-1.5 ml-auto" :class="waLink ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30' : 'bg-slate-700 text-slate-500 cursor-not-allowed'"><i data-lucide="message-circle" class="w-4 h-4"></i> Enviar WA</a>
...
<div class="text-center"><span class="text-slate-500 block">Precio B2B</span><span class="text-amber-400 font-bold text-sm" x-text="ldCalcPrecio.precio_b2b ? ldCalcPrecio.precio_b2b+'€/par' : '-'"></span></div>
<div class="text-center"><span class="text-slate-500 block">Facturacion</span><span class="text-emerald-400 font-bold text-sm" x-text="ldCalcPrecio.facturacion ? ldCalcPrecio.facturacion+'€' : '-'"></span></div>
<div class="text-center"><span class="text-slate-500 block">Margen Club</span><span class="text-blue-400 font-bold text-sm" x-text="ldCalcPrecio.margen_total ? ldCalcPrecio.margen_total+'€' : '-'"></span></div>
<div class="col-span-3 text-center text-slate-500 text-[10px]" x-text="ldCalcPrecio.tramo||''"></div>
</div>
...
<!-- ═══ MOTIVO PERDIDA ═══ -->
<div x-show="ld.estado_lead === '09 Perdido'" class="bg-rose-500/10 border border-rose-500/30 rounded-lg p-3">
...
<!-- ═══ MOCKUP ═══ -->
<div x-show="['05 Cualificado','06 Propuesta','07 Negociacion'].includes(ld.estado_lead)" class="bg-indigo-500/10 border border-indigo-500/30 rounded-xl p-4 space-y-3">
...
<div class="flex justify-between"><span class="text-slate-500">Estado:</span><span class="font-bold" :class="ldMockup.estado==='enviado'?'text-emerald-400':ldMockup.estado==='solicitado'?'text-amber-400':'text-slate-400'" x-text="ldMockup.estado||'pendiente'"></span></div>
<div class="flex justify-between" x-show="ldMockup.solicitado_en"><span class="text-slate-500">Solicitado:</span><span class="text-slate-300" x-text="ldMockup.solicitado_en?.substring(0,16)"></span></div>
<div class="flex justify-between" x-show="ldMockup.enviado_en"><span class="text-slate-500">Enviado:</span><span class="text-slate-300" x-text="ldMockup.enviado_en?.substring(0,16)"></span></div>
</div>
...
<!-- ═══ PRESUPUESTO ═══ -->
<template x-if="['06 Propuesta','07 Negociacion'].includes(ld.estado_lead)">
...
<div class="flex justify-between"><span class="text-slate-500">Version:</span><span class="text-slate-300 font-bold" x-text="'v'+ld.presupuesto.version"></span></div>
<div class="flex justify-between"><span class="text-slate-500">Unidades:</span><span class="text-slate-300" x-text="ld.presupuesto.unidades"></span></div>
<div class="flex justify-between"><span class="text-slate-500">Precio Unitario:</span><span class="text-amber-400" x-text="ld.presupuesto.precio_unitario+'€'"></span></div>
<div class="flex justify-between"><span class="text-slate-500">Condiciones:</span><span class="text-slate-300" x-text="ld.presupuesto.condiciones_pago"></span></div>
<div class="flex justify-between border-t border-slate-700 pt-1 mt-1"><span class="text-slate-500">Total:</span><span class="text-emerald-400 font-bold text-sm" x-text="ld.presupuesto.importe_total+'€'"></span></div>
<div class="flex justify-between"><span class="text-slate-500">Margen Club:</span><span class="text-blue-400" x-text="ld.presupuesto.margen_potencial_club+'€'"></span></div>
<div class="flex justify-between"><span class="text-slate-500">Fecha:</span><span class="text-slate-300" x-text="ld.presupuesto?.fecha?.substring(0,16) || ''"></span></div>
</div>
</template>
</div>
</template>
...
<button @click="guardarFicha()" :disabled="!ldChanged" class="px-5 py-2.5 rounded-lg text-sm font-bold transition flex items-center gap-2" :class="ldChanged ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30 hover:bg-amber-500/30 cursor-pointer' : 'bg-slate-800 text-slate-600 border border-slate-700 cursor-not-allowed'">
...
<div class="px-5 py-3 border-b border-slate-800 flex items-center justify-between"><h5 class="text-sm font-bold text-slate-200">Anadir Nuevo Lead (con validacion MX)</h5><button @click="al=false" class="text-slate-500 hover:text-slate-300"><i data-lucide="x" class="w-5 h-5"></i></button></div>
...
<div class="sticky top-0 bg-slate-900 border-b border-slate-800 px-5 py-3 flex items-center justify-between rounded-t-2xl z-10">
<div class="flex items-center gap-2"><i data-lucide="bar-chart-3" class="w-4 h-4 text-amber-400"></i><h5 class="text-sm font-bold text-slate-200" x-text="aqTitulo(aqTab)"></h5><span x-show="aqData" class="text-xs text-slate-500 ml-2" x-text="aqData.total + ' registros'"></span><span x-show="aqData && aqData.hoy !== undefined" class="text-xs text-blue-400 ml-2" x-text="'(hoy: '+aqData.hoy+')'"></span><span x-show="aqLoading" class="text-xs text-slate-500 ml-2">Cargando...</span></div>
<div class="flex items-center gap-1">
<button @click="abrirAnalytics('envios')" class="px-2.5 py-1 rounded text-xs font-semibold transition" :class="aqTab==='envios'?'bg-blue-500/20 text-blue-400':'text-slate-500 hover:text-slate-300'">Envios</button>
<button @click="abrirAnalytics('aperturas')" class="px-2.5 py-1 rounded text-xs font-semibold transition" :class="aqTab==='aperturas'?'bg-cyan-500/20 text-cyan-400':'text-slate-500 hover:text-slate-300'">Aperturas</button>
<button @click="abrirAnalytics('rebotes')" class="px-2.5 py-1 rounded text-xs font-semibold transition" :class="aqTab==='rebotes'?'bg-rose-500/20 text-rose-400':'text-slate-500 hover:text-slate-300'">Rebotes</button>
<button @click="abrirAnalytics('bajas')" class="px-2.5 py-1 rounded text-xs font-semibold transition" :class="aqTab==='bajas'?'bg-amber-500/20 text-amber-400':'text-slate-500 hover:text-slate-300'">Bajas</button>
<button @click="aq=false" class="text-slate-500 hover:text-slate-300 ml-2"><i data-lucide="x" class="w-4 h-4"></i></button>
</div>
</div>
