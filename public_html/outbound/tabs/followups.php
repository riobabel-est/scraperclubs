...
<div class="flex items-center justify-between"><span class="text-xs text-slate-400 uppercase tracking-wider">No Respondedores</span><i data-lucide="mail-question" class="w-4 h-4 text-slate-500"></i></div>
<div class="text-2xl font-bold text-rose-400 mt-1" x-text="kpis.no_respondedores"></div>
<div class="text-[10px] text-slate-500 mt-1">Contactados sin respuesta</div>
</div>
<div class="bg-slate-900 border border-amber-500/20 rounded-xl p-4">
<div class="flex items-center justify-between"><span class="text-xs text-slate-400 uppercase tracking-wider">Sin Prox. Accion</span><i data-lucide="alert-circle" class="w-4 h-4 text-slate-500"></i></div>
<div class="text-2xl font-bold text-amber-400 mt-1" x-text="kpis.sin_proxima_accion"></div>
<div class="text-[10px] text-slate-500 mt-1">Leads operativos sin proxima accion</div>
</div>
<div class="bg-slate-900 border border-indigo-500/20 rounded-xl p-4">
<div class="flex items-center justify-between"><span class="text-xs text-slate-400 uppercase tracking-wider">Mockups Pend.</span><i data-lucide="palette" class="w-4 h-4 text-slate-500"></i></div>
<div class="text-2xl font-bold text-indigo-400 mt-1" x-text="kpis.mockups_pendientes"></div>
<div class="text-[10px] text-slate-500 mt-1">Solicitados / en produccion</div>
</div>
<div class="bg-slate-900 border border-emerald-500/20 rounded-xl p-4">
<div class="flex items-center justify-between"><span class="text-xs text-slate-400 uppercase tracking-wider">Presup. Pend.</span><i data-lucide="file-text" class="w-4 h-4 text-slate-500"></i></div>
<div class="text-2xl font-bold text-emerald-400 mt-1" x-text="kpis.presupuestos_pendientes"></div>
<div class="text-[10px] text-slate-500 mt-1">Presupuestos en estado creado</div>
</div>
</div>
...
<td class="px-2 py-1.5 text-right text-slate-500" x-text="l.num_envios"></td>
<td class="px-2 py-1.5 text-center"><span class="px-1.5 py-0.5 rounded-full text-[10px] font-bold" :class="l.tiene_apertura ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-700 text-slate-500'" x-text="l.tiene_apertura ? 'Si' : 'No'"></span></td>
...
<template x-if="noRespondedores.length === 0">
<div class="text-xs text-slate-500 py-8 text-center">No hay leads no respondedores. ¡Buen trabajo!</div></template>
</div>
...
<td class="px-2 py-1.5 text-right" :class="l.presupuesto_importe ? 'text-emerald-400 font-bold' : 'text-slate-500'" x-text="l.presupuesto_importe ? l.presupuesto_importe + '\u20AC' : '-'"></td>
...
<template x-if="sinProximaAccion.length === 0">
<div class="text-xs text-slate-500 py-8 text-center">Todos los leads operativos tienen proxima accion. ¡Excelente!</div></template>
</div>
